...
func schemaCatalog() map[string]schemaInfo {
	return map[string]schemaInfo{
		...
		"wait": {
			Name:        "wait",
			Description: "Page condition wait result for text, selector, locator, JavaScript expression, load-state, request, response, network-idle, dialog, or file-chooser checks.",
			Fields: []schemaField{
				{Name: "ok", Type: "boolean", Required: true, Description: "True when the condition matched before timeout."},
				{Name: "target", Type: "page", Required: true, Description: "Selected page target metadata."},
				{Name: "wait", Type: "wait_result", Required: true, Description: "Kind, condition or CDP event method, load state and readyState when applicable, criteria, match status, bounded evidence, observed event counts, network-idle in-flight evidence and warnings, dialog handling state, file-chooser interception state, elapsed time, timeout, and poll interval when applicable."},
				...
				{Name: "last_event", Type: "network_dialog_or_file_chooser_wait_event", Required: false, Description: "Last observed request/response candidate, network-idle lifecycle event, dialog event, or file chooser event."},
