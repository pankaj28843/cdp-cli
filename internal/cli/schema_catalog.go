...
func schemaCatalog() map[string]schemaInfo {
	catalog := map[string]schemaInfo{
		...
		"auto-heal-environment": {
			...
			Fields: []schemaField{
				...
				{Name: "last_observed_at", Type: "string", Required: false, Description: "Last owner-only environment observation timestamp."},
				{Name: "suppress_until", Type: "string", Required: false, Description: "Cooldown end timestamp after a wake/suspend gap."},
				{Name: "retry_after_seconds", Type: "number", Required: false, Description: "Minimum suggested delay before another launch-capable Auto Heal attempt."},
				{Name: "reason", Type: "string", Required: true, Description: "Stable privacy-safe decision reason."},
				{Name: "checked_at", Type: "string", Required: true, Description: "UTC timestamp for this environment decision."},
			},
		},
		"daemon-maintenance": {
			...
			Fields: []schemaField{
				...
				{Name: "environment", Type: "auto_heal_environment", Required: false, Description: "Host availability gate used before live managed-headless maintenance; offline and post-wake results skip mutating phases."},
		...
		"events-stream":        {Name: "events-stream", Description: "Persistent exact-session CDP event stream with bounded liveness emitted as one JSON record per line.", Fields: []schemaField{{Name: "ok", Type: "boolean", Required: true, Description: "Whether this JSONL record represents a successful stream state or event."}, {Name: "type", Type: "string", Required: true, Description: "JSONL record type: ready, event, subscription, stopped, or error."}, {Name: "target", Type: "page", Required: false, Description: "Selected page target metadata on the ready record."}, {Name: "event", Type: "cdp_event", Required: false, Description: "Raw CDP event from only the attached target session."}, {Name: "stream", Type: "object", Required: false, Description: "Session binding, target selection, event bounds, enabled domains, active subscriptions, and liveness configuration."}, {Name: "liveness", Type: "object", Required: false, Description: "Metadata-only runtime-registration and exact-session heartbeat configuration; definitive runtime replacement may set state retired, reason runtime_retired, and source runtime_registration, while ambiguous state falls through to heartbeat."}, {Name: "operation", Type: "string", Required: false, Description: "Subscription operation: add or remove."}, {Name: "method", Type: "string", Required: false, Description: "Subscription method on a subscription record."}, {Name: "reason", Type: "string", Required: false, Description: "Why the stream stopped."}, {Name: "event_count", Type: "number", Required: false, Description: "Number of matching events emitted before stopping."}, {Name: "foreign_events_dropped", Type: "number", Required: false, Description: "Events received for another CDP session and discarded."}, {Name: "truncated", Type: "boolean", Required: false, Description: "Whether the max-events bound stopped the stream."}, {Name: "all_events", Type: "boolean", Required: false, Description: "Whether the stream is in wildcard mode; a minus subscription excludes one method."}, {Name: "code", Type: "string", Required: false, Description: "Stable error code on an error record."}, {Name: "err_class", Type: "string", Required: false, Description: "Stable error class on an error record."}, {Name: "message", Type: "string", Required: false, Description: "Actionable error message on an error record."}, {Name: "remediation_commands", Type: "array<string>", Required: false, Description: "Safe follow-up commands on an error record."}}},
		...
		"events-interactions":  {Name: "events-interactions", Description: "Bounded exact-session DOM interaction observer adapted from the Runtime.addBinding collaboration bridge with metadata-only output.", Fields: []schemaField{{Name: "ok", Type: "boolean", Required: true, Description: "Whether this JSONL record is a valid observer state or sanitized interaction."}, {Name: "type", Type: "string", Required: true, Description: "JSONL record type: ready, interaction, or stopped."}, {Name: "target", Type: "page", Required: false, Description: "Selected exact page target metadata."}, {Name: "observer", Type: "object", Required: false, Description: "Session, target, kind filters, bounds, current/future listener installation, and sanitized-payload contract."}, {Name: "event", Type: "cdp_event", Required: false, Description: "Safe method-only Runtime.bindingCalled marker for composing with events wait; raw binding payload is never returned."}, {Name: "interaction", Type: "object", Required: false, Description: "Allow-listed click, scroll, selectionchange, or keydown metadata without page text, key values, input values, HTML, cookies, or arbitrary binding fields."}, {Name: "reason", Type: "string", Required: false, Description: "Why observation stopped."}, {Name: "event_count", Type: "number", Required: false, Description: "Number of emitted sanitized interactions."}, {Name: "foreign_events_dropped", Type: "number", Required: false, Description: "Binding events from another session discarded by the observer."}, {Name: "ignored_binding_events", Type: "number", Required: false, Description: "Malformed or unsafe binding payloads dropped without output."}, {Name: "cleanup", Type: "object", Required: false, Description: "Whether current listeners, future-document script, and binding were removed."}, {Name: "truncated", Type: "boolean", Required: false, Description: "Whether max-events stopped observation."}}},
	...
	catalog["events-stream"] = schemaInfo{
		...
		Description: "Persistent exact-session CDP event stream with validated target-domain enablement and bounded exact-session liveness emitted as one JSON record per line.",
