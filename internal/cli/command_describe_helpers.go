...
func commandExamples(path string) []string {
	examples := map[string][]string{
		...
		"cdp page cleanup": {
			...
			"cdp --browser-mode headless page cleanup --created-by cdp --idle-for 30m --close --force --json",
		...
		"cdp workflow action-capture": {
			"cdp workflow action-capture --action click:'button.submit' --include network,console,dom,text --wait-after 2s --evidence-out-dir tmp/action-capture --json",
