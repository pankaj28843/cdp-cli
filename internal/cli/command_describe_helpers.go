...
func commandExamples(path string) []string {
	examples := map[string][]string{
		...
		"cdp page close": {
			...
			"cdp page close --target 2 --wait-gone --json",
	...
	examples["cdp dialog"] = []string{"cdp dialog accept --wait --type prompt --prompt-text yes --target-index 2 --json", "cdp dialog dismiss --target-index 2 --json"}
	examples["cdp dialog accept"] = []string{"cdp dialog accept --prompt-text yes --json", "cdp dialog accept --wait --type prompt --prompt-text yes --json", "cdp dialog accept --wait --target-index 2 --json", "cdp dialog accept --prompt-text yes --target-index 2 --json"}
	examples["cdp dialog dismiss"] = []string{"cdp dialog dismiss --json", "cdp dialog dismiss --wait --type confirm --target-index 2 --json", "cdp dialog dismiss --target-index 2 --json"}
	examples["cdp emulate"] = []string{"cdp emulate viewport --preset mobile --json", "cdp emulate viewport --preset mobile --target-index 2 --json", "cdp emulate user-agent --user-agent 'Mozilla/5.0 ...' --json", "cdp emulate timezone --timezone-id UTC --json", "cdp emulate locale --locale de-DE --json", "cdp emulate color-scheme --scheme dark --json", "cdp emulate network --preset slow-3g --json"}
	examples["cdp emulate viewport"] = []string{"cdp emulate viewport --width 390 --height 844 --mobile --dpr 1 --json", "cdp emulate viewport --preset iphone-12 --target-index 2 --json"}
	examples["cdp emulate clear"] = []string{"cdp emulate clear --json", "cdp emulate clear --target-index 2 --json"}
	examples["cdp emulate media"] = []string{"cdp emulate media --prefers-color-scheme dark --json", "cdp emulate media --prefers-color-scheme dark --target-index 2 --json"}
	examples["cdp emulate color-scheme"] = []string{"cdp emulate color-scheme --scheme dark --json", "cdp emulate color-scheme --scheme light --target-index 2 --json"}
	examples["cdp emulate user-agent"] = []string{"cdp emulate user-agent --user-agent 'Mozilla/5.0 (X11; Linux x86_64) AppleWebKit/537.36 Chrome/125 Safari/537.36' --platform Linux --json", "cdp emulate user-agent --user-agent 'Mozilla/5.0 ...' --target-index 2 --json"}
	examples["cdp emulate geolocation"] = []string{"cdp emulate geolocation --latitude 55.6761 --longitude 12.5683 --accuracy 100 --json", "cdp emulate geolocation --latitude 55.6761 --longitude 12.5683 --target-index 2 --json"}
	examples["cdp emulate timezone"] = []string{"cdp emulate timezone --timezone-id UTC --json", "cdp emulate timezone --timezone-id America/New_York --target-index 2 --json"}
	examples["cdp emulate locale"] = []string{"cdp emulate locale --locale de-DE --json", "cdp emulate locale --locale en-US --target-index 2 --json"}
	examples["cdp emulate cpu"] = []string{"cdp emulate cpu --rate 4 --json", "cdp emulate cpu --rate 1 --target-index 2 --json"}
	examples["cdp emulate network"] = []string{"cdp emulate network --preset slow-3g --json", "cdp emulate network --latency 100 --download-kbps 750 --upload-kbps 250 --target-index 2 --json", "cdp emulate network --preset none --json"}
	...
	examples["cdp events stream"] = []string{
		...
		"cdp events stream --target 2 --enable page,runtime --duration 30s --json",
	...
	examples["cdp events interactions"] = []string{
		...
		"cdp events interactions --target 2 --match click --max-events 1 --json",
	}
