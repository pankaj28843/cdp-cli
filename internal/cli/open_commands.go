...
func (a *app) newOpenCommand() *cobra.Command {
	...
	cmd.Flags().StringVar(&urlContains, "url-contains", "", "navigate the unique page whose URL contains this text when --new-tab=false")
	cmd.Flags().StringVar(&titleContains, "title-contains", "", "navigate the unique page whose title contains this text when --new-tab=false")
