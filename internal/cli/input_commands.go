...
func (a *app) newClickCommand() *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if waitFileChooser {
				fileChooserCriteria = fileChooserWaitCriteria{Mode: waitFileChooserMode}
				if err := normalizeFileChooserWaitCriteria(&fileChooserCriteria); err != nil {
					return err
				}
				teardown, err := setupFileChooserWait(ctx, client, session.SessionID)
				if err != nil {
			...
			if waitFileChooser {
				observation, err := collectFileChooserEvent(ctx, client, session.SessionID, fileChooserCriteria)
				fileChooserReport = fileChooserWaitReport(observation, fileChooserCriteria, time.Since(eventWaitStart), a.effectiveNetworkWaitTimeout())
				fileChooserReport["target"] = pageRow(target)
				verified = observation.Matched
				result.Verified = &verified
				if err != nil {
					fileChooserErr = err
...
func addFileChooserWaitToClickReport(report map[string]any, fileChooserReport map[string]any) {
	if report == nil || fileChooserReport == nil {
		return
	}
	if wait, ok := fileChooserReport["wait"]; ok {
		report["file_chooser_wait"] = wait
	}
	if chooser, ok := fileChooserReport["file_chooser"]; ok {
		report["file_chooser"] = chooser
	}
	if lastEvent, ok := fileChooserReport["last_event"]; ok {
		report["last_file_chooser_event"] = lastEvent
	}
...
func (a *app) newTypeCommand() *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if stateWaitCount > 1 {
				return commandError("usage", "usage", "use only one type wait mode: --wait-text, --wait-selector, --wait-url, or --wait-url-contains", ExitUsage, []string{"cdp type 'Search' query --by label --wait-url-contains /results --json"})
			}
			if poll <= 0 {
			...
			if trial && stateWaitCount > 0 {
				return commandError("usage", "usage", "--trial does not change the page, so it cannot use type wait flags", ExitUsage, []string{"cdp type 'Search' query --by label --wait-url-contains /results --json"})
			}
			ctx, cancel := a.browserCommandContext(cmd)
			defer cancel()
...
func (a *app) newPressCommand() *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if stateWaitCount > 1 {
				return commandError("usage", "usage", "use only one press wait mode: --wait-text, --wait-selector, --wait-url, or --wait-url-contains", ExitUsage, []string{"cdp press Enter 'Search' --by label --wait-url-contains /results --json"})
			}
			if poll <= 0 {
			...
			if trial && stateWaitCount > 0 {
				return commandError("usage", "usage", "--trial does not dispatch key events, so it cannot use press wait flags", ExitUsage, []string{"cdp press Enter 'Search' --by label --wait-url-contains /results --json"})
			}
