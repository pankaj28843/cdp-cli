...
func capabilityCatalog() []map[string]any {
	return []map[string]any{
		...
		capabilityRow("emulation", "implemented", "viewport, media, user-agent, geolocation, CPU throttling, responsive audit", []string{"cdp emulate viewport --help", "cdp emulate user-agent --help", "cdp emulate geolocation --help", "cdp emulate cpu --help"}, []string{"cdp workflow responsive-audit https://example.com --json"}),
		capabilityRow("network_throttling", "planned", "network throttling presets", []string{"cdp protocol describe Network.emulateNetworkConditions --json"}, []string{"cdp workflow responsive-audit https://example.com --include network --json"}),
	}
}
