...
const (
	AuthRefreshSchemaVersion   = "perplexity-auth-refresh/v1"
	DoctorSchemaVersion        = "perplexity-doctor/v1"
		...
		func(
			...
			if err != nil {
				return authFailure(
					...
					"perplexity_auth_prepare_failed", "connection",
					"Perplexity auth observation could not prepare the exact headed target",
					data,
				)
			}
			if err := lease.MarkPrepared(ctx); err != nil {
				return authFailure(
					runID, config, webagent.StageAttached, target, pending,
					"perplexity_auth_prepare_state_failed", "internal",
					"Perplexity auth observation preparation could not be persisted",
					data,
				)
			}
			...
			if found {
				...
				return authFailure(
					runID, config, webagent.StageObserveTerminal, target, pending,
					"perplexity_list_request_not_observed", "auth",
					"Signed-in Perplexity conversation-list request evidence was not observed",
					data,
				)
			}
			if err := config.Store.SaveTemplate(ctx, template); err != nil {
				...
				return authFailure(
					runID, config, webagent.StageObserveTerminal, target, pending,
					"perplexity_auth_state_write_failed", "internal",
					"Perplexity request template could not be persisted to owner-only state",
					data,
				)
			}
			if err := lease.MarkTerminal(ctx); err != nil {
				return authFailure(
					runID, config, webagent.StageObserveTerminal, target, pending,
					"perplexity_auth_terminal_state_failed", "internal",
					"Perplexity auth terminal state could not be persisted",
					data,
				)
			}
			...
			return operationSuccess(
				...
				[]string{
					"cdp workflow agent perplexity capabilities refresh --json",
...
func Doctor(
	...
	if auth.Ready && runtime.Ready {
		...
		data.StableReads = "candidate_http"
	...
	code := "perplexity_auth_" + auth.State
	errClass := "auth"
	message := "Perplexity auth evidence is not ready"
	next := []string{"cdp workflow agent perplexity auth refresh --json"}
	if auth.Ready && !runtime.Ready {
		code = "perplexity_runtime_capabilities_" + runtime.State
		errClass = "capability"
		message = "Perplexity runtime capability evidence is not ready"
		next = []string{"cdp workflow agent perplexity capabilities refresh --json"}
	}
...
func parseConversationListURL(rawURL string) (*url.URL, bool) {
	...
	if err != nil ||
		...
		parsed.User != nil ||
		parsed.Fragment != "" ||
		parsed.Query().Get("version") == "" ||
		parsed.Query().Get("source") == "" {
		return nil, false
	}
	return parsed, true
}
...
func readCookies(ctx context.Context, session *cdp.PageSession) (map[string]string, error) {
	raw, err := session.Exec(
		ctx,
		"Network.getCookies",
		json.RawMessage(`{"urls":["https://www.perplexity.ai"]}`),
	)
	if err != nil {
		return nil, err
	}
	...
	if err := json.Unmarshal(raw, &payload); err != nil {
		return nil, fmt.Errorf("decode Perplexity cookies")
	}
...
func hasSessionCookie(cookies map[string]string) bool {
	for name, value := range cookies {
		if (strings.HasPrefix(name, "__Secure-next-auth.session-token") ||
			strings.HasPrefix(name, "__Secure-pplx.session")) &&
			value != "" {
			return true
		}
	}
