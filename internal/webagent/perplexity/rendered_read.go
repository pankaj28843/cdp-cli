...
func renderedListConversations(
	...
	if err := prepareRenderedPage(
		...
		return renderedReadFailure(
			...
			target, pending,
			readFailure{
				code:     "perplexity_rendered_list_unavailable",
				errClass: "connection",
				message:  "Perplexity rendered conversation history could not be loaded",
			},
			map[string]any{"schema_version": ConversationListSchemaVersion},
			nil,
		)
	...
	if err != nil {
		...
		return renderedReadFailure(
			...
			target, pending,
			readFailure{
				code:     "perplexity_rendered_sidebar_not_ready",
				errClass: "provider",
				message:  "Perplexity rendered history sidebar did not become uniquely ready",
			},
			map[string]any{
	...
	if !observation.Expanded {
		...
		if clickErr != nil || outcome.Dispatch != browserflow.DispatchPerformed {
			...
			return renderedReadFailure(
				...
				target, pending,
				readFailure{
					code:     "perplexity_rendered_sidebar_open_failed",
					errClass: "provider",
					message:  "Perplexity rendered history sidebar was not opened once",
				},
				map[string]any{
					"schema_version": ConversationListSchemaVersion,
					"attempts":       attempts,
	...
	if !observation.Expanded || stableReads < 3 && len(observation.Conversations) < limit {
		...
		return renderedReadFailure(
			...
			target, pending,
			readFailure{
				code:     "perplexity_rendered_list_incomplete",
				errClass: "completion",
				message:  "Perplexity rendered conversation history did not reach a stable snapshot",
			},
			map[string]any{
				"schema_version": ConversationListSchemaVersion,
	...
	if len(conversations) == 0 {
		...
		return renderedReadFailure(
			...
			target, pending,
			readFailure{
				code:     "perplexity_rendered_list_empty",
				errClass: "completion",
				message:  "Perplexity rendered history links were not present",
			},
			map[string]any{
				"schema_version": ConversationListSchemaVersion,
				"stable_reads":   stableReads,
	...
	if err := lease.MarkPrepared(ctx); err != nil {
		return renderedReadFailure(
			runID, config.BuildCommit, operation, webagent.StageAttached,
			target, pending,
			*internalReadFailure("Perplexity rendered list state could not be persisted"),
			data,
			nil,
		)
	}
	if err := lease.MarkTerminal(ctx); err != nil {
		return renderedReadFailure(
			...
			target, pending,
			*internalReadFailure("Perplexity rendered list terminal state could not be persisted"),
			data,
			nil,
		)
	}
...
func renderedConversationDetail(
	...
	if err := prepareRenderedPage(
		...
		return renderedReadFailure(
			...
			target, pending,
			readFailure{
				code:     "perplexity_rendered_detail_unavailable",
				errClass: "connection",
				message:  "Perplexity rendered conversation detail could not be loaded",
			},
			map[string]any{"schema_version": ConversationDetailSchemaVersion},
			conversationRef(conversationID),
		)
	...
	if strings.TrimSpace(observation.Prompt) != "" {
		data.promptText = observation.Prompt
	...
	if err := lease.MarkPrepared(ctx); err != nil {
		return renderedReadFailure(
			runID, config.BuildCommit, operation, webagent.StageAttached,
			target, pending,
			*internalReadFailure("Perplexity rendered detail state could not be persisted"),
			data,
			conversationRef(conversationID),
		)
	}
	if state == webagent.StateTerminal {
		if err := lease.MarkTerminal(ctx); err != nil {
			return renderedReadFailure(
				runID, config.BuildCommit, operation, webagent.StageObserveTerminal,
				target, pending,
				*internalReadFailure("Perplexity rendered detail terminal state could not be persisted"),
				data,
				conversationRef(conversationID),
			)
		}
...
func validateRenderedReadConfig(config RenderedReadConfig) *readFailure {
	if config.Client == nil || config.Engine == nil || config.Journal == nil {
		return internalReadFailure(
			"Perplexity rendered fallback transaction is not configured",
		)
	}
	return nil
}
	...
	if err != nil ||
		...
		*result = replaceFailure(
			*result,
			...
			"cleanup",
			"Perplexity rendered fallback could not prove exact target cleanup",
...
func renderedAcquireFailure(
	...
	failure := readFailure{
		code:     "perplexity_rendered_target_unavailable",
		errClass: "connection",
		message:  "Perplexity rendered fallback could not acquire one exact headed target",
	}
	var budget *browserflow.BudgetExceededError
	if errors.As(err, &budget) {
		failure.code = "perplexity_browser_resource_budget_exceeded"
		failure.errClass = "resource"
		failure.message = "Perplexity rendered fallback was blocked by the headed browser resource budget"
	}
	return renderedReadFailure(
		runID, config.BuildCommit, operation, webagent.StagePlanned,
		nil, webagent.CleanupEvidence{State: webagent.CleanupNotRequired},
