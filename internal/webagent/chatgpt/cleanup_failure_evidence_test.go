...
func TestCleanupRetainsOriginalOperationFailure(t *testing.T) {
	for _, failed := range []bool{false, true} {
		t.Run(map[bool]string{false: "success", true: "failure"}[failed], func(t *testing.T) {
			...
			result := runOwned(context.Background(), BrowserConfig{Client: client, Engine: engine, Journal: journal, BuildCommit: "test"},
				...
				func(_ *browserflow.Lease, _ *webagent.TargetEvidence, _ webagent.CleanupEvidence) webagent.Result {
					...
					if failed {
						r.Error = &webagent.OperationError{Code: "chatgpt_composer_not_ready", ErrClass: "provider", Message: "private-message-marker", Reason: "private-reason-marker"}
					}
					return r
				})
			if result.Error == nil || result.Error.Code != "chatgpt_exact_target_cleanup_failed" || result.OK || result.Stage != webagent.StageCleanupPending {
			...
			if failed && (original == nil || original.Code != "chatgpt_composer_not_ready" || original.ErrClass != "provider" || original.Stage != webagent.StageAttached) {
...
func TestAcquireFailureSurvivesCleanupFailure(t *testing.T) {
	...
	if result.Error == nil || result.Error.Code != "chatgpt_exact_target_cleanup_failed" {
		t.Fatalf("primary error: %+v", result.Error)
	}
	original := result.Evidence.OperationFailure
	if original == nil || original.Code != "chatgpt_browser_start_failed" || original.ErrClass != "connection" || original.Stage != webagent.StagePlanned {
