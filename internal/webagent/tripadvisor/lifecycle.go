...
func runOwned(
	...
	if config.Client == nil ||
		...
		return operationFailure(
			...
			nil, nil,
			"tripadvisor_browserflow_unavailable", "internal",
			"Tripadvisor exact-target browser transaction is not configured",
			"", data,
	...
	if err != nil {
		...
		code := "tripadvisor_browser_start_failed"
		errClass := "connection"
		message := "Tripadvisor workflow could not acquire one exact headed target"
		var budget *browserflow.BudgetExceededError
		if errors.As(err, &budget) {
			code = "tripadvisor_browser_resource_budget_exceeded"
			errClass = "resource"
			message = "Tripadvisor workflow was blocked by the headed browser resource budget"
		}
		if cleanup.State == webagent.CleanupFailed {
			code = "tripadvisor_exact_target_cleanup_failed"
			errClass = "cleanup"
			message = "Tripadvisor workflow could not prove exact target cleanup"
		}
		return operationFailure(
	...
	defer func() {
		...
		if closeErr != nil ||
			...
			result = replaceFailure(
				result,
				"tripadvisor_exact_target_cleanup_failed",
				"cleanup",
				"Tripadvisor workflow could not prove exact target cleanup",
...
func preparePage(
	...
	if err := cdp.ActivateTargetWithClient(
		ctx,
		client,
		session.TargetID,
	); err != nil {
