...
func runOwned(
	...
	if config.Client == nil ||
		...
		return operationFailure(
			...
			"Grok exact-target browser transaction is not configured", "",
			data, []string{"cdp workflow agent grok doctor --json"},
		)
	}
	...
	if err != nil {
		code := "grok_admission_unavailable"
		errClass := "internal"
		message := "Grok provider admission state is unavailable"
		retryAt := ""
		nextCommands := []string{"cdp workflow agent grok doctor --json"}
		...
		if errors.As(err, &blocked) {
			...
			if blocked.ResolutionNeeded {
				nextCommands = []string{"cdp workflow agent admission status grok --json"}
			} else {
	...
	if err != nil {
		...
		code := "grok_browser_start_failed"
		errClass := "connection"
		message := "Grok workflow could not acquire one exact headed target"
		var budget *browserflow.BudgetExceededError
		if errors.As(err, &budget) {
			code = "grok_browser_resource_budget_exceeded"
			errClass = "resource"
			message = "Grok workflow was blocked by the headed browser resource budget"
		}
		if cleanup.State == webagent.CleanupFailed {
			code = "grok_exact_target_cleanup_failed"
			errClass = "cleanup"
			message = "Grok workflow could not prove exact target cleanup"
		}
		return operationFailure(
	...
	defer func() {
		...
		if closeErr != nil || cleanup.State != browserflow.CleanupClosed || !cleanup.TargetGone {
			...
			result = replaceFailure(
				...
				"cleanup",
				"Grok workflow could not prove exact target cleanup",
...
func cleanupCommands(runID string, cleanup webagent.CleanupEvidence) []string {
	commands := []string{"cdp workflow agent grok doctor --json"}
