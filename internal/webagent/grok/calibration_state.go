...
const (
	CalibrationStateSchemaVersion = "grok-calibration-state/v1"
	RelativeCalibrationStatePath  = "webagent/grok/calibration.json"
)
...
func NewCalibrationStore(stateDir string) (*CalibrationStore, error) {
	...
	if stateDir == "" {
		return nil, fmt.Errorf("Grok calibration state directory is required")
	}
...
func (s *CalibrationStore) Save(
	...
	if s == nil || s.path == "" {
		return fmt.Errorf("Grok calibration store is not configured")
	}
...
func (s *CalibrationStore) Load(
	...
	if s == nil || s.path == "" {
		return CalibrationStateRecord{}, false, fmt.Errorf(
			"Grok calibration store is not configured",
		)
	}
...
func (r CalibrationStateRecord) Validate() error {
	...
	if !safeCalibrationIdentity.MatchString(r.RunID) {
		return fmt.Errorf("invalid Grok calibration run_id")
	}
	switch r.State {
	...
	default:
		return fmt.Errorf("invalid Grok calibration state %q", r.State)
	}
	if len(r.PromptFingerprint) != 64 {
		return fmt.Errorf(
			"Grok calibration prompt fingerprint must be SHA-256",
		)
	}
	if r.TargetID != "" &&
		!safeCalibrationIdentity.MatchString(r.TargetID) {
		return fmt.Errorf("invalid Grok calibration target_id")
	}
	if r.ConversationID != "" &&
		!conversationIDPattern.MatchString(r.ConversationID) {
		return fmt.Errorf("invalid Grok calibration conversation_id")
	}
	...
	if r.Postcondition != "" &&
		...
		return fmt.Errorf("invalid Grok calibration postcondition")
	}
...
func CalibrationStatus(
	...
	if store == nil {
		return calibrationMetadataFailure(
			...
			"internal",
			"Grok calibration state is unavailable",
			data,
		)
	}
...
func CleanupCalibration(
	...
	if config.Store == nil || config.Journal == nil {
		return calibrationMetadataFailure(
			config.BuildCommit,
			"grok_calibration_cleanup_unavailable",
			"internal",
			"Grok calibration cleanup is not configured",
	...
	if !record.TargetClosed && record.TargetID != "" {
		if config.Engine == nil {
			return calibrationCleanupFailure(
				config.BuildCommit,
				record,
				"grok_calibration_target_cleanup_unavailable",
				"cleanup",
				"Grok calibration exact target cleanup is not configured",
				nil,
		...
		if recoverErr != nil ||
			...
			return calibrationCleanupFailure(
				config.BuildCommit,
				record,
				"grok_calibration_target_cleanup_failed",
				"cleanup",
				"Grok calibration exact target cleanup could not be proved",
				nil,
				webagent.CleanupEvidence{
					Required: true,
	...
	if err := config.Store.Save(ctx, record); err != nil {
		return calibrationMetadataFailure(
			...
			"grok_calibration_state_unavailable",
			"internal",
			"Grok calibration cleanup state could not be persisted",
			calibrationStatusData(record),
		)
	}
	...
	if record.ConversationID == "" {
		...
		return calibrationCleanupFailure(
			config.BuildCommit,
			record,
			"grok_calibration_identity_unavailable",
			"completion",
			"Grok calibration has no exact acknowledged conversation identity; no delete was attempted",
	...
	if record.DeleteDispatch == browserflow.DispatchPerformed ||
		...
		return calibrationCleanupFailure(
			config.BuildCommit,
			record,
			"grok_calibration_delete_ambiguous",
			"completion",
			"Grok calibration deletion was already attempted without a terminal postcondition; it will not be repeated",
	...
	if err := config.Store.Save(context.Background(), record); err != nil {
		return calibrationMetadataFailure(
			...
			"grok_calibration_state_unavailable",
			"internal",
			"Grok calibration cleanup outcome could not be persisted",
			calibrationStatusData(record),
		)
	}
