	...
	return runOwned(
		...
		func(
			...
			if err != nil {
				...
				return authFailure(
					runID, config, webagent.StagePrepared, target, pending,
					"grok_cookie_observation_failed", "connection",
					"Grok cookie evidence could not be read from the exact headed target",
					data,
				)
			}
			...
			if !data.SessionCookieObserved {
				...
				return authFailure(
					...
					"grok_signed_out", "auth",
					"Signed-in Grok cookie evidence was not observed",
					data,
				)
			}
			now := time.Now
