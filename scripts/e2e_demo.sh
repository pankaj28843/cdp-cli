...
"$binary" assert checked "Subscribe to newsletter" --by label --state-dir "$state_dir/cdp-state" --json \
  | jq -e '.ok == true and .assertion.passed == true and .assertion.checked == true and .assertion.unchecked == false and .assertion.checked_count == 1 and .assertion.unchecked_count == 0 and .assertion.unsupported_count == 0 and .locator.strict == true and .resolved_selector == "input#subscribe" and .assertion.items[0].role == "checkbox"' >/dev/null
...
"$binary" assert unchecked "Subscribe to newsletter" --by label --state-dir "$state_dir/cdp-state" --json \
  | jq -e '.ok == true and .assertion.passed == true and .assertion.checked == false and .assertion.unchecked == true and .assertion.checked_count == 0 and .assertion.unchecked_count == 1 and .locator.strict == true and .resolved_selector == "input#subscribe"' >/dev/null
