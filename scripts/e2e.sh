...
"$binary" describe --command "type" --json | jq -e '.ok == true and .commands.name == "type" and (.commands.examples | any(contains("--by label"))) and (.commands.examples | any(contains("--trial"))) and (.commands.examples | any(contains("--force"))) and (.commands.flags[] | select(.name == "by")) and (.commands.flags[] | select(.name == "role")) and (.commands.flags[] | select(.name == "trial")) and (.commands.flags[] | select(.name == "force"))' >/dev/null
...
"$binary" schema type --json | jq -e '.ok == true and .schema.name == "type" and (.schema.fields | map(.name) | index("type")) and (.schema.fields | map(.name) | index("actionability")) and (.schema.fields | map(.name) | index("locator")) and (.schema.fields | map(.name) | index("resolved_selector"))' >/dev/null
