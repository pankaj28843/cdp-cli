...
"$binary" describe --command "workflow rendered-extract" --json | jq -e '.ok == true and .commands.name == "rendered-extract" and (.commands.examples | any(contains("--serp google"))) and (.commands.examples | any(contains("--target") and contains("--reload"))) and (.commands.examples | any(contains("--wait 20s") and contains("--settle 2s"))) and (.commands.flags[] | select(.name == "target")) and (.commands.flags[] | select(.name == "url-contains")) and (.commands.flags[] | select(.name == "title-contains")) and (.commands.flags[] | select(.name == "reload")) and (.commands.flags[] | select(.name == "settle" and .default == "2s")) and (.commands.flags[] | select(.name == "min-visible-words" and (.usage | contains("readiness")) and (.usage | contains("0 to disable")))) and (.commands.flags[] | select(.name == "out-dir"))' >/dev/null
...
"$binary" describe --command "workflow web-research extract" --json | jq -e '.ok == true and .commands.name == "extract" and (.commands.examples | any(contains("--parallel 4") and contains("--settle 2s"))) and (.commands.examples | any(contains("--parallel 10"))) and (.commands.flags[] | select(.name == "url-file")) and (.commands.flags[] | select(.name == "settle" and .default == "2s")) and (.commands.flags[] | select(.name == "min-html-chars" and (.usage | contains("quality")) and (.usage | contains("0 to disable"))))' >/dev/null
