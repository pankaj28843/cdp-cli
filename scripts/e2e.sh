...
"$binary" schema open --json | jq -e '.ok == true and .schema.name == "open" and (.schema.fields | map(.name) | index("page")) and (.schema.fields | map(.name) | index("attempts")) and (.schema.fields | map(.name) | index("retry_policy"))' >/dev/null
...
"$binary" describe --command "open" --json | jq -e '.ok == true and .commands.name == "open" and (.commands.examples | any(contains("--retry transient"))) and (.commands.flags[] | select(.name == "retry")) and (.commands.flags[] | select(.name == "max-attempts"))' >/dev/null
