...
"$binary" describe --command "workflow submit-search" --json | jq -e '.ok == true and .commands.name == "submit-search" and (.commands.examples | any(contains("--wait-url-contains"))) and (.commands.examples | any(contains("--suggestion"))) and (.commands.examples | any(contains("--wait-load-state"))) and (.commands.examples | any(contains("--submit none"))) and (.commands.flags[] | select(.name == "input-mode")) and (.commands.flags[] | select(.name == "submit")) and (.commands.flags[] | select(.name == "suggestion")) and (.commands.flags[] | select(.name == "suggestion-by")) and (.commands.flags[] | select(.name == "wait-url-contains")) and (.commands.flags[] | select(.name == "wait-load-state"))' >/dev/null
...
"$binary" schema workflow-submit-search --json | jq -e '.ok == true and .schema.name == "workflow-submit-search" and (.schema.fields | map(.name) | index("fill")) and (.schema.fields | map(.name) | index("press")) and (.schema.fields | map(.name) | index("suggestion")) and (.schema.fields | map(.name) | index("suggestion_click")) and (.schema.fields | map(.name) | index("verification")) and (.schema.fields | map(.name) | index("before_target")) and (.schema.fields | map(.name) | index("final_target")) and (.schema.fields[] | select(.name == "workflow").description | contains("suggestion_requested")) and (.schema.fields[] | select(.name == "suggestion").description | contains("strictness")) and (.schema.fields[] | select(.name == "verification").description | contains("--wait-load-state"))' >/dev/null
