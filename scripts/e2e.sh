...
"$binary" describe --command "press" --json | jq -e '.ok == true and .commands.name == "press" and (.commands.examples | any(contains("--by label"))) and (.commands.examples | any(contains("--trial"))) and (.commands.flags[] | select(.name == "by")) and (.commands.flags[] | select(.name == "role")) and (.commands.flags[] | select(.name == "trial"))' >/dev/null
...
"$binary" schema press --json | jq -e '.ok == true and .schema.name == "press" and (.schema.fields | map(.name) | index("press")) and (.schema.fields | map(.name) | index("actionability")) and (.schema.fields | map(.name) | index("locator"))' >/dev/null
