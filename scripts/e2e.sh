...
"$binary" schema eval --json | jq -e '.ok == true and .schema.name == "eval" and (.schema.fields | map(.name) | index("result")) and (.schema.fields | map(.name) | index("attempt_count")) and (.schema.fields | map(.name) | index("last_observed_target"))' >/dev/null
...
"$binary" schema daemon-health-check --json | jq -e '.ok == true and .schema.name == "daemon-health-check" and (.schema.fields | map(.name) | index("status")) and (.schema.fields | map(.name) | index("usable")) and (.schema.fields | map(.name) | index("degraded_reasons")) and (.schema.fields | map(.name) | index("recommended_action")) and (.schema.fields | map(.name) | index("steps")) and (.schema.fields | map(.name) | index("repair")) and (.schema.fields | map(.name) | index("artifacts")) and (.schema.fields | map(.name) | index("failure_count"))' >/dev/null
...
"$binary" describe --command "open" --json | jq -e '.ok == true and .commands.name == "open" and (.commands.examples | any(contains("--retry transient"))) and (.commands.examples | any(contains("--task-id"))) and (.commands.examples | any(contains("--reuse"))) and (.commands.examples | any(contains("--budget-summary"))) and (.commands.flags[] | select(.name == "retry")) and (.commands.flags[] | select(.name == "max-attempts")) and (.commands.flags[] | select(.name == "reuse")) and (.commands.flags[] | select(.name == "budget-summary")) and (.commands.flags[] | select(.name == "run-id")) and (.commands.flags[] | select(.name == "task-id")) and (.commands.flags[] | select(.name == "root-task-id")) and (.commands.flags[] | select(.name == "parent-task-id")) and (.commands.flags[] | select(.name == "created-by"))' >/dev/null
...
"$binary" describe --command "scroll" --json | jq -e '.ok == true and .commands.name == "scroll" and (.commands.examples | any(contains("--by role"))) and (.commands.examples | any(contains("--trial"))) and (.commands.examples | any(contains("--block"))) and (.commands.flags[] | select(.name == "by")) and (.commands.flags[] | select(.name == "role")) and (.commands.flags[] | select(.name == "trial")) and (.commands.flags[] | select(.name == "block")) and (.commands.flags[] | select(.name == "inline"))' >/dev/null
"$binary" schema scroll --json | jq -e '.ok == true and .schema.name == "scroll" and (.schema.fields | map(.name) | index("scroll")) and (.schema.fields | map(.name) | index("actionability")) and (.schema.fields | map(.name) | index("resolved_selector"))' >/dev/null
...
"$binary" describe --command "eval" --json | jq -e '.ok == true and .commands.name == "eval" and (.commands.examples | any(contains("--title-contains"))) and (.commands.examples | any(contains("--retry transient"))) and (.commands.flags[] | select(.name == "retry")) and (.commands.flags[] | select(.name == "max-attempts"))' >/dev/null
...
"$binary" describe --command "wait url" --json | jq -e '.ok == true and .commands.name == "url" and (.commands.examples | any(contains("--mode contains"))) and (.commands.examples | any(contains("--mode exact"))) and (.commands.flags[] | select(.name == "mode")) and (.commands.flags[] | select(.name == "poll")) and (.commands.flags[] | select(.name == "url-contains"))' >/dev/null
...
"$binary" describe --command "wait eval" --json | jq -e '.ok == true and .commands.name == "eval" and (.commands.examples | any(contains("__rendered"))) and (.commands.examples | any(contains("--ready-expr"))) and (.commands.examples | any(contains("--retry transient"))) and (.commands.flags[] | select(.name == "ready-expr")) and (.commands.flags[] | select(.name == "ready-field")) and (.commands.flags[] | select(.name == "out-dir")) and (.commands.flags[] | select(.name == "retry")) and (.commands.flags[] | select(.name == "max-attempts"))' >/dev/null
"$binary" describe --command "wait load-state" --json | jq -e '.ok == true and .commands.name == "load-state" and (.commands.examples | any(contains("domcontentloaded"))) and (.commands.flags[] | select(.name == "poll"))' >/dev/null
...
"$binary" describe --command "network block" --json | jq -e '.ok == true and .commands.name == "block" and (.commands.examples | any(contains("--pattern") and contains("--duration")))' >/dev/null
"$binary" describe --command "network mock" --json | jq -e '.ok == true and .commands.name == "mock" and (.commands.examples | any(contains("--rule") and contains("max_matches")))' >/dev/null
"$binary" describe --command "workflow page-load" --json | jq -e '.ok == true and .commands.name == "page-load" and (.commands.examples | any(contains("--reload"))) and (.commands.flags[] | select(.name == "ready-file"))' >/dev/null
"$binary" describe --command "workflow rendered-extract" --json | jq -e '.ok == true and .commands.name == "rendered-extract" and (.commands.examples | any(contains("--serp google"))) and (.commands.examples | any(contains("arxiv.org/pdf/") and contains("--content-extractor auto"))) and (.commands.examples | any(contains("news.ycombinator.com/item") and contains("--content-extractor auto"))) and (.commands.examples | any(contains("--target") and contains("--reload"))) and (.commands.examples | any(contains("--wait 20s") and contains("--settle 2s"))) and (.commands.flags[] | select(.name == "target")) and (.commands.flags[] | select(.name == "url-contains")) and (.commands.flags[] | select(.name == "title-contains")) and (.commands.flags[] | select(.name == "reload")) and (.commands.flags[] | select(.name == "settle" and .default == "2s")) and (.commands.flags[] | select(.name == "selector" and (.usage | contains("generic capture/fallback")) and (.usage | contains("Hacker News")))) and (.commands.flags[] | select(.name == "content-extractor" and .default == "auto" and (.usage | contains("native source profiles")))) and (.commands.flags[] | select(.name == "min-visible-words" and (.usage | contains("readiness")) and (.usage | contains("0 to disable")))) and (.commands.flags[] | select(.name == "out-dir"))' >/dev/null
"$binary" describe --command "workflow pdf-to-markdown" --json | jq -e '.ok == true and .commands.name == "pdf-to-markdown" and (.commands.examples | any(contains("--wait-download") and contains("cdp open") and contains(".page.target_id") and contains("--target \"$pdf_target\"") and contains("page close --target \"$pdf_target\""))) and (.commands.flags[] | select(.name == "out-dir" and ((has("default") | not) or .default == "")))' >/dev/null
...
"$binary" describe --command "workflow web-research serp" --json | jq -e '.ok == true and .commands.name == "serp" and (.commands.examples | any(contains("--serp google") and contains("cdr:1,cd_min:07/01/2026,cd_max:07/01/2026"))) and (.commands.examples | any(contains("--navigation-delay 30s") and contains("--parallel 1") and contains("--blocked-failure-threshold 1"))) and (.commands.examples | any(contains("--serp all"))) and (.commands.examples | any(contains("--parallel-engines"))) and (.commands.examples | any(contains("--serp duckduckgo"))) and (.commands.examples | any(contains("--fallback-serp google"))) and (.commands.examples | any(contains("--result-pages 3") and contains("--settle 2s"))) and (.commands.examples | any(contains("--fast-fail-blocked"))) and (.commands.examples | any(contains("--progress stderr"))) and (.commands.flags[] | select(.name == "query-file" and (.usage | contains("query<TAB>Google tbs time filter")) and (.usage | contains("# comment rows ignored")))) and (.commands.flags[] | select(.name == "serp" and (.usage | contains("comma-separated")))) and (.commands.flags[] | select(.name == "parallel-engines")) and (.commands.flags[] | select(.name == "fallback-serp" and (.usage | contains("auto")))) and (.commands.flags[] | select(.name == "candidate-out")) and (.commands.flags[] | select(.name == "settle" and .default == "2s")) and (.commands.flags[] | select(.name == "navigation-delay" and .default == "0s" and (.usage | contains("minimum delay")) and (.usage | contains("engine lane")))) and (.commands.flags[] | select(.name == "result-pages")) and (.commands.flags[] | select(.name == "fast-fail-blocked")) and (.commands.flags[] | select(.name == "blocked-failure-threshold")) and (.commands.flags[] | select(.name == "progress"))' >/dev/null
...
"$binary" describe --command "workflow perf" --json | jq -e '.ok == true and .commands.name == "perf" and (.commands.examples | any(contains("--trace-max-bytes") and contains("--redact safe"))) and (.commands.flags[] | select(.name == "trace-max-bytes")) and (.commands.flags[] | select(.name == "redact"))' >/dev/null
...
"$binary" describe --command "workflow debug-bundle" --json | jq -e '.ok == true and .commands.name == "debug-bundle" and (.commands.examples | any(contains("--task-id"))) and (.commands.examples | any(contains("--inline-payloads"))) and (.commands.examples | any(contains("--reload=false") and contains("--ignore-cache=false"))) and (.commands.flags[] | select(.name == "redact")) and (.commands.flags[] | select(.name == "inline-payloads")) and (.commands.flags[] | select(.name == "reload" and .default == "true")) and (.commands.flags[] | select(.name == "ignore-cache" and .default == "true")) and (.commands.flags[] | select(.name == "run-id")) and (.commands.flags[] | select(.name == "task-id")) and (.commands.flags[] | select(.name == "stage"))' >/dev/null
"$binary" describe --command "workflow action-capture" --json | jq -e '.ok == true and .commands.name == "action-capture" and (.commands.examples | any(contains("--include network,console,dom,text,a11y,screenshot"))) and (.commands.examples | any(contains("--include-bodies json,text") and contains("--body-url-contains"))) and (.commands.flags[] | select(.name == "evidence-out-dir" and (.usage | contains("accessibility")) and (.usage | contains("screenshot")) and (.usage | contains("manifest")))) and (.commands.flags[] | select(.name == "include" and (.usage | contains("a11y")) and (.usage | contains("screenshot")))) and (.commands.flags[] | select(.name == "include-bodies" and .default == "none")) and (.commands.flags[] | select(.name == "body-limit")) and (.commands.flags[] | select(.name == "body-url-contains" and (.usage | contains("--include-bodies")))) and (.commands.flags[] | select(.name == "screenshot-full-page")) and (.commands.flags[] | select(.name == "a11y-depth")) and (.commands.flags[] | select(.name == "a11y-limit"))' >/dev/null
"$binary" describe --command "workflow submit-search" --json | jq -e '.ok == true and .commands.name == "submit-search" and (.commands.examples | any(contains("--wait-url-contains"))) and (.commands.examples | any(contains("--suggestion"))) and (.commands.examples | any(contains("--wait-load-state"))) and (.commands.examples | any(contains("--wait-response"))) and (.commands.examples | any(contains("--submit none"))) and (.commands.flags[] | select(.name == "input-mode")) and (.commands.flags[] | select(.name == "submit")) and (.commands.flags[] | select(.name == "suggestion")) and (.commands.flags[] | select(.name == "suggestion-by")) and (.commands.flags[] | select(.name == "wait-url-contains")) and (.commands.flags[] | select(.name == "wait-load-state")) and (.commands.flags[] | select(.name == "wait-response")) and (.commands.flags[] | select(.name == "wait-response-match-url")) and (.commands.flags[] | select(.name == "wait-response-status"))' >/dev/null
...
"$binary" schema text --json | jq -e '.ok == true and .schema.name == "text" and (.schema.fields | map(.name) | index("attempts")) and (.schema.fields | map(.name) | index("retry_policy"))' >/dev/null
...
"$binary" schema workflow-rendered-extract --json | jq -e '.ok == true and .schema.name == "workflow-rendered-extract" and (.schema.fields | map(.name) | index("content")) and (.schema.fields | map(.name) | index("quality")) and (.schema.fields | map(.name) | index("artifacts"))' >/dev/null
...
"$binary" schema workflow-web-research-serp --json | jq -e '.ok == true and .schema.name == "workflow-web-research-serp" and (.schema.fields[] | select(.name == "queries" and .type == "array<web_research_query>").description | contains("query<TAB>") and contains("applied only to Google") and contains("cdr:1,cd_min:07/01/2026,cd_max:07/01/2026")) and (.schema.fields | map(.name) | index("candidates")) and (.schema.fields | map(.name) | index("query_coverage")) and (.schema.fields | map(.name) | index("warnings")) and (.schema.fields | map(.name) | index("failures")) and (.schema.fields | map(.name) | index("artifacts")) and (.schema.fields[] | select(.name == "workflow").description | contains("reusable engine lanes"))' >/dev/null
...
"$binary" schema workflow-perf --json | jq -e '.ok == true and .schema.name == "workflow-perf" and (.schema.fields | map(.name) | index("performance")) and (.schema.fields | map(.name) | index("trace")) and (.schema.fields | map(.name) | index("insights")) and (.schema.fields[] | select(.name == "trace").description | contains("artifact-safety"))' >/dev/null
...
"$binary" schema workflow-debug-bundle --json | jq -e '.ok == true and .schema.name == "workflow-debug-bundle" and (.schema.fields | map(.name) | index("bundle")) and (.schema.fields | map(.name) | index("artifacts")) and (.schema.fields | map(.name) | index("artifact_list")) and (.schema.fields[] | select(.name == "bundle").description | contains("public-safe")) and (.schema.fields[] | select(.name == "requests").description | contains("--inline-payloads")) and (.schema.fields[] | select(.name == "artifacts").description | contains("local_only"))' >/dev/null
...
"$binary" schema hover --json | jq -e '.ok == true and .schema.name == "hover" and (.schema.fields | map(.name) | index("hover")) and (.schema.fields | map(.name) | index("actionability")) and (.schema.fields | map(.name) | index("auto_scroll")) and (.schema.fields | map(.name) | index("locator")) and (.schema.fields | map(.name) | index("resolved_selector"))' >/dev/null
"$binary" schema drag --json | jq -e '.ok == true and .schema.name == "drag" and (.schema.fields | map(.name) | index("drag")) and (.schema.fields | map(.name) | index("actionability")) and (.schema.fields | map(.name) | index("auto_scroll")) and (.schema.fields | map(.name) | index("locator")) and (.schema.fields | map(.name) | index("resolved_selector"))' >/dev/null
...
"$binary" describe --command "events tap" --json | jq -e '.ok == true and .commands.name == "tap" and (.commands.flags[] | select(.name == "max-events")) and (.commands.flags[] | select(.name == "ready-file"))' >/dev/null
