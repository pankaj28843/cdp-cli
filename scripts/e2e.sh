...
"$binary" describe --command "wait eval" --json | jq -e '.ok == true and .commands.name == "eval" and (.commands.examples | any(contains("__rendered"))) and (.commands.examples | any(contains("--ready-expr"))) and (.commands.flags[] | select(.name == "ready-expr")) and (.commands.flags[] | select(.name == "ready-field")) and (.commands.flags[] | select(.name == "out-dir"))' >/dev/null
