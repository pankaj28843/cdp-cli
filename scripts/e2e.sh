...
"$binary" describe --command "workflow action-capture" --json | jq -e '.ok == true and .commands.name == "action-capture" and (.commands.examples | any(contains("--evidence-out-dir"))) and (.commands.flags[] | select(.name == "evidence-out-dir"))' >/dev/null
...
"$binary" schema workflow-action-capture --json | jq -e '.ok == true and .schema.name == "workflow-action-capture" and (.schema.fields | map(.name) | index("evidence")) and (.schema.fields[] | select(.name == "evidence").description | contains("--evidence-out-dir"))' >/dev/null
