...
"$binary" describe --command "events stream" --json | jq -e '.ok == true and .commands.name == "stream" and (.commands.examples | any(contains("events stream"))) and (.commands.examples | any(contains("--target 2"))) and (.commands.examples | any(contains("--target-index"))) and (.commands.flags[] | select(.name == "target-index" and .type == "int")) and (.commands.flags[] | select(.name == "max-events" and .type == "int")) and (.commands.flags[] | select(.name == "target" and (.usage | contains("numeric"))))' >/dev/null
"$binary" describe --command "events stream" --json | jq -e '[(.commands.flags[] | select(.name == "target-id" or .name == "url") | .type)] | sort == ["string", "string"]' >/dev/null
...
"$binary" describe --command "events interactions" --json | jq -e '.ok == true and .commands.name == "interactions" and (.commands.examples | any(contains("--match click,scroll"))) and (.commands.examples | any(contains("--target 2"))) and (.commands.flags[] | select(.name == "target-index" and .type == "int")) and (.commands.flags[] | select(.name == "match")) and (.commands.flags[] | select(.name == "ready-file")) and (.commands.flags[] | select(.name == "target" and (.usage | contains("numeric"))))' >/dev/null
"$binary" describe --command "events interactions" --json | jq -e '[(.commands.flags[] | select(.name == "target-id" or .name == "url") | .type)] | sort == ["string", "string"]' >/dev/null
"$binary" schema events-interactions --json | jq -e '.ok == true and .schema.name == "events-interactions" and (.schema.description | contains("liveness")) and (.schema.fields | map(.name) | index("interaction")) and (.schema.fields | map(.name) | index("cleanup")) and (.schema.fields[] | select(.name == "observer").description | contains("exact-session liveness")) and (.schema.fields[] | select(.name == "interaction").description | contains("key values"))' >/dev/null
...
chatgpt_research_export_help="$("$binary" workflow agent chatgpt conversations export-research --help)"
grep -q 'export-research CONVERSATION_ID' <<<"$chatgpt_research_export_help"
grep -q -- '--output string' <<<"$chatgpt_research_export_help"
...
"$binary" describe --command "workflow agent chatgpt conversations export-research" --json | jq -e '.ok == true and .commands.name == "export-research" and (.commands.use | contains("export-research CONVERSATION_ID")) and (.commands.flags[] | select(.name == "output" and .type == "string")) and (.commands.flags[] | select(.name == "overwrite" and .type == "bool"))' >/dev/null
...
"$binary" --state-dir "$state_dir" workflow agent chatgpt capabilities --json | jq -e '.ok == true and .provider == "chatgpt" and (.data.operations[] | select(.operation == "conversations.export_research" and .supported == true and .status == "implemented" and .side_effect == "local_file_write" and .browser == "headed" and (.command | endswith("conversations export-research"))))' >/dev/null
...
"$binary" explain-error invalid_target_index --json | jq -e '.ok == true and .error.code == "invalid_target_index" and .error.err_class == "usage" and .error.exit_code == 2 and (.error.remediation_commands | index("cdp pages --json") != null)' >/dev/null
...
"$binary" schema chatgpt-research-export --json | jq -e '.ok == true and .schema.name == "chatgpt-research-export" and ((.schema.fields | map(.name) | sort) == ["conversation_id", "exported_bytes", "metadata", "output_path", "read_mode", "schema_version", "sha256"]) and ((.schema.fields | map(.name) | index("report")) == null)' >/dev/null
...
"$binary" schema protocol-exec --json | jq -e '.ok == true and .schema.name == "protocol-exec" and (.schema.description | contains("1-based target index")) and (.schema.description | contains("pre-execution selected-schema validation")) and (.schema.fields | map(.name) | index("scope")) and (.schema.fields | map(.name) | index("artifact")) and (.schema.fields[] | select(.name == "target" and .type == "target" and (.description | contains("target-index"))))' >/dev/null
...
"$binary" schema protocol-domains --json | jq -e '.ok == true and .schema.name == "protocol-domains" and (.schema.fields | map(.name) | index("source"))' >/dev/null
"$binary" schema protocol-search --json | jq -e '.ok == true and .schema.name == "protocol-search" and (.schema.fields | map(.name) | index("source"))' >/dev/null
"$binary" schema protocol-describe --json | jq -e '.ok == true and .schema.name == "protocol-describe" and (.schema.fields | map(.name) | index("source"))' >/dev/null
...
printf '%s\n' "$bare_protocol_shape_output" | jq -e '.ok == false and .code == "invalid_json" and (.message | contains("JSON object"))' >/dev/null
...
if [[ "${CDP_E2E_AUTO_CONNECT:-}" == "1" || "${CDP_E2E_AUTO_CONNECT:-}" == "true" ]]; then
  ...
  if [[ "${CDP_E2E_ACTIVE_BROWSER:-}" == "1" || "${CDP_E2E_ACTIVE_BROWSER:-}" == "true" ]]; then
    ...
    set +e
    live_root_exec_output="$("$binary" --timeout 5s Browser.getVersion '{}' --json 2>/tmp/cdp-cli-live-root-exec.err)"
    live_root_exec_code=$?
    set -e
    if [[ "$live_root_exec_code" -eq 0 ]]; then
      printf '%s\n' "$live_root_exec_output" | jq -e '.ok == true and .method == "Browser.getVersion"' >/dev/null
    else
      printf '%s\n' "$live_root_exec_output" | jq -e '.ok == false and (.code == "connection_failed" or .code == "connection_not_configured")' >/dev/null
    fi
    set +e
