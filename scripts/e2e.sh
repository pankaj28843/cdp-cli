...
"$binary" describe --command "workflow submit-search" --json | jq -e '.ok == true and .commands.name == "submit-search" and (.commands.examples | any(contains("--wait-url-contains"))) and (.commands.examples | any(contains("--suggestion"))) and (.commands.examples | any(contains("--submit none"))) and (.commands.flags[] | select(.name == "input-mode")) and (.commands.flags[] | select(.name == "submit")) and (.commands.flags[] | select(.name == "suggestion")) and (.commands.flags[] | select(.name == "suggestion-by")) and (.commands.flags[] | select(.name == "wait-url-contains"))' >/dev/null
