...
"$binary" describe --command "workflow action-capture" --json | jq -e '.ok == true and .commands.name == "action-capture" and (.commands.examples | any(contains("--include network,console,dom,text,a11y"))) and (.commands.flags[] | select(.name == "evidence-out-dir" and (.usage | contains("accessibility")))) and (.commands.flags[] | select(.name == "a11y-depth")) and (.commands.flags[] | select(.name == "a11y-limit"))' >/dev/null
