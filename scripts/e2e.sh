...
"$binary" describe --command "workflow submit-search" --json | jq -e '.ok == true and .commands.name == "submit-search" and (.commands.examples | any(contains("--wait-url-contains"))) and (.commands.examples | any(contains("--submit none"))) and (.commands.flags[] | select(.name == "input-mode")) and (.commands.flags[] | select(.name == "submit")) and (.commands.flags[] | select(.name == "wait-url-contains"))' >/dev/null
...
"$binary" schema workflow-submit-search --json | jq -e '.ok == true and .schema.name == "workflow-submit-search" and (.schema.fields | map(.name) | index("fill")) and (.schema.fields | map(.name) | index("press")) and (.schema.fields | map(.name) | index("verification")) and (.schema.fields | map(.name) | index("before_target")) and (.schema.fields | map(.name) | index("final_target")) and (.schema.fields[] | select(.name == "workflow").description | contains("input_mode")) and (.schema.fields[] | select(.name == "verification").description | contains("--wait-url-contains"))' >/dev/null
