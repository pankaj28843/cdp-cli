...
"$binary" describe --command "workflow rendered-extract" --json | jq -e '.ok == true and .commands.name == "rendered-extract" and (.commands.examples | any(contains("--serp google"))) and (.commands.examples | any(contains("--target") and contains("--reload"))) and (.commands.flags[] | select(.name == "target")) and (.commands.flags[] | select(.name == "url-contains")) and (.commands.flags[] | select(.name == "title-contains")) and (.commands.flags[] | select(.name == "reload")) and (.commands.flags[] | select(.name == "out-dir"))' >/dev/null
...
"$binary" schema workflow-web-research-serp --json | jq -e '.ok == true and .schema.name == "workflow-web-research-serp" and (.schema.fields[] | select(.name == "queries" and .type == "array<web_research_query>").description | contains("query<TAB>") and contains("applied only to Google") and contains("cdr:1,cd_min:07/01/2026,cd_max:07/01/2026")) and (.schema.fields | map(.name) | index("candidates")) and (.schema.fields | map(.name) | index("warnings")) and (.schema.fields | map(.name) | index("failures")) and (.schema.fields | map(.name) | index("artifacts")) and (.schema.fields[] | select(.name == "workflow").description | contains("reusable engine lanes"))' >/dev/null
