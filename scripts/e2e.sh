...
"$binary" schema open --json | jq -e '.ok == true and .schema.name == "open" and (.schema.fields | map(.name) | index("page")) and (.schema.fields | map(.name) | index("attempts")) and (.schema.fields | map(.name) | index("retry_policy")) and (.schema.fields | map(.name) | index("run_id")) and (.schema.fields | map(.name) | index("task_id")) and (.schema.fields | map(.name) | index("target_task_ids"))' >/dev/null
...
"$binary" describe --command "open" --json | jq -e '.ok == true and .commands.name == "open" and (.commands.examples | any(contains("--retry transient"))) and (.commands.examples | any(contains("--task-id"))) and (.commands.flags[] | select(.name == "retry")) and (.commands.flags[] | select(.name == "max-attempts")) and (.commands.flags[] | select(.name == "run-id")) and (.commands.flags[] | select(.name == "task-id")) and (.commands.flags[] | select(.name == "root-task-id")) and (.commands.flags[] | select(.name == "parent-task-id")) and (.commands.flags[] | select(.name == "created-by"))' >/dev/null
